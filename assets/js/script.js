...
async function getImages() {
    const datas = await fecthingData()


    setImagesInPage(datas)
    setHeroImage(datas)
}

async function setImagesInPage(datas) {
    thumbnails.innerHTML = ''
    datas.forEach((image) => {
        thumbnails.innerHTML += `
       <div class="thumb" >
          <img src="${image.imagem.url}">
      </div>
        `
...
async function setHeroImage(datas){
    datas.forEach((image)=>{
        if(image.fotoDeFundo == true){
            
            hero.style.backgroundImage = `linear-gradient(rgba(0, 0, 0), rgba(0, 0, 0, 0.1)), url(${image.imagem.url})`
        }
    })
}
